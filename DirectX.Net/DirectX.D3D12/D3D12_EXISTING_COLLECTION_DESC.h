...
namespace DirectX
{
    namespace D3D12
    {
        ...
        public value struct D3D12_EXISTING_COLLECTION_DESC
        {
            IntPtr pExistingCollection;//ID3D12StateObject
            unsigned int NumExports;
            IntPtr pExports; //D3D12_EXPORT_DESC*

            property ID3D12StateObject^ ExistingCollection
            {
                ID3D12StateObject^ get()
                {
                    return gcnew ID3D12StateObject(pExistingCollection);
                }
            }
